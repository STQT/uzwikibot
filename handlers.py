...
async def start_menu_handler(m: types.Message, bot: Bot):
    if await configs.collusers.count_documents({"_id": m.from_user.id}) == 0:
        # Adding new user DB
        await configs.collusers.insert_one({"_id": m.from_user.id,
                                            "username": m.from_user.username,
                                            "first_name": m.from_user.first_name,
                                            "last_name": m.from_user.last_name})
    elif await configs.collusers.count_documents(
            {"_id": m.from_user.id, "status": False}) == 1:
        await configs.collusers.update_one(
...
async def some_text_handler(message: types.Message, bot: Bot):
    if message.chat.id == configs.GROUP_ID and message.reply_to_message:
        report_data = await configs.collreports.find_one({
    ...
    async with state.proxy() as data:
        if message.text == "✅ Submit":
            ...
            if data['message'].content_type == 'voice':
                ...
                await configs.collreports.insert_one({
